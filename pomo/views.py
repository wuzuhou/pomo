...
def signup():
    if request.method == 'POST':
        ...
        if User.query.filter_by(email=email).first():
            ...
            db.session.add(user)
            db.session.commit()
            emailcontent={'subject': '番茄土豆账户激活',
            'text1': '欢迎使用番茄土豆，请点击下面的链接来激活您的账户',
            'text2': 'active',
            'text3':'账户激活'}
            emailverify(email, emailcontent)
            return render_template('toactive.html')
...
def forgot():
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first()
        if user:
            emailcontent={'subject': '番茄土豆密码重设邮件',
            'text1': '您申请了重设密码，请点击下面的链接来重设密码',
            'text2': 'reset',
            'text3':'重置密码',}
            activestrings = uuid.uuid4().hex
            user.activestrings = activestrings
...
def emailverify(email, emailcontent):
